...
import { Markdown, MessageResponse, MessageResponseMode, ResponsesStreamResponse } from '@tg-sensei/bot';
...
import { getTorrentResponse } from 'telegram-bot/responses/torrent-client/torrents/torrent/item';
...
export function getAddTorrentResponse(
  ...
  return new ResponsesStreamResponse(async function* () {
    ...
    if (!torrent) {
      yield new MessageResponse({
        mode,
        content: 'Данные торрента отсутствуют',
      });
    ...
    yield new MessageResponse({
      mode,
      content: Markdown.create`Торрент${torrent.name ? Markdown.create` "${torrent.name}"` : ''} добавлен!`,
    });

    yield getTorrentResponse(torrent.infoHash, {
      mode,
    });
  });
}
